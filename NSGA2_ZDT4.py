...
def zdt4(individual):
    ...
    """
    g  = 1 + 10*(len(individual)-1) + sum(xi**2 - 10*math.cos(4*np.pi*xi) for xi in individual[1:])
    f1 = individual[0]
...
plt.title("first population of ZDT4")
plt.figure()
...
while(gen_no<max_gen):
    ...
    for valuez in ndf[0]:
        ...
        sum_i=np.sum(solution_all[valuez])
        sum_final=sum_final+sum_i
        print("\n")
    subtraction=sum_final-sum_old
    print("subtraction",subtraction)
    sum_old=sum_final
    print(solution_all[ndf[0][0]])
    z_first=solution_all[ndf[0][0]]
    print("first solution: ",np.sum(z_first[1:]))
    sum_z_first=np.sum(z_first[1:])

    z_last = solution_all[ndf[0][-1]]
    print("last solution: ", np.sum(z_last[1:]))
    sum_z_last = np.sum(z_last[1:])
    sub=sum_z_last-sum_z_first
    print("sub",sub)
    print("\n")
    ...
    for i in range(0,len(ndf)):
        ...
        if len(non_dominated)==1:
            crowding_distance_values.append(np.array([0]))
        else:
    ...
    while(len(solution2)!=2*pop_size):
        ...
        c1 = random.randint(0, pop_size - 1)
        result=crossX(solution_all[a1], solution_all[b1])
        # print("result",len(result))
        solution2.append(result[:10])

        # if (np.abs(sub)==0 or np.abs(sub)<0.001):
        #
        #     result_mu = mutation(solution_all[c1])
        #     print("mu",result_mu)
        #     solution2.append(result_mu)
...
while(gen_no<max_gen):
    ...
    sum_old=0
    for valuez in ndf[0]:
        solution_all1[valuez]=[round(elem,3) for elem in solution_all1[valuez]]
        print(solution_all1[valuez],end=" ")
        sum_i=np.sum(solution_all1[valuez])
        sum_final=sum_final+sum_i
        print("\n")
    subtraction=sum_final-sum_old
    print("subtraction",subtraction)
    sum_old=sum_final
    print(solution_all1[ndf[0][0]])
    z_first=solution_all1[ndf[0][0]]
    print("first solution: ",np.sum(z_first[1:]))
    sum_z_first=np.sum(z_first[1:])

    z_last = solution_all1[ndf[0][-1]]
    print("last solution: ", np.sum(z_last[1:]))
    sum_z_last = np.sum(z_last[1:])
    sub=sum_z_last-sum_z_first
    print("sub",sub)
    ...
    for i in range(0,len(ndf)):
        ...
        if len(non_dominated)==1:
            ...
            crowding_distance_values.append(pg.crowding_distance(points=f))
        # print("crowding_dis: ",crowding_distance_values)
    solution2 = solution_all1[:][:]
    #Generating offsprings
    while(len(solution2)!=2*pop_size):
        a1 = random.randint(0,pop_size-1)
        b1 = random.randint(0,pop_size-1)
        c1 = random.randint(0, pop_size - 1)
        result=crossX(solution_all1[a1], solution_all1[b1])
        # print("result",len(result))
        solution2.append(result[:10])

        if (np.abs(sub)==0 or np.abs(sub)<0.001):

            result_mu = mutation(solution_all1[c1])
            print("mu",result_mu)
            solution2.append(result_mu)
    ...
    for x in new_solution:
        print(x)
        solution_all1.append(solution2[x])
        print(len(solution2[x]))
        print(solution2[x])
...
F_2=np.empty((len(function2_values),2))
for i in range(len(function2_values)):
    if function2_values[i]<5:
        F_2[i, 0] = function1_values[i]
        F_2[i, 1] = function2_values[i]
...
plot.add(F_2,facecolor="none", edgecolor="red")
plot.show()